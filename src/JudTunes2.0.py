...
class JudTunes(commands.Cog):
    ...
    async def play(self, ctx, *track):
        ...
        if not track:
            ...
            # elif ctx.voice_client and self.playlist:
            #     await self.play_audio(ctx, self.playlist.pop(0))
            # else:
            ...
            # Try to play the song or add to queue
            try:
    ...
    async def ensure_voice(self, ctx):
        if ctx.voice_client is None:
            if ctx.author.voice:
                ...
                self.bot.loop.create_task(self.start_playing(ctx.voice_client))
                print("Created play task")
            else:
...
KEY = "OTI1ODgyMjg5OTc4NzU3MjAw.YczlEw.8FVSprTGvmbcbpxNFdAh0MV-7eI"
